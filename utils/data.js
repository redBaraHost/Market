import { v4 as uuidv4 } from 'uuid';

export const recommander= [
  {
    id: uuidv4(),
    name: 'Pixel 8 pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/E2J7hG3ZRWpFOO0NBrkg1s6K1ZKYLwEuCK_lAO7LrPVH3C0eTbFCycYgqs4gr8caAr27S-PKLy2yLhIVGCy4byZ5wZ8kzQpnyQk=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Pixel Buds pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '149',
    last_prix: '229',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/9G5VkratxCGLyYy5NLfL2-AMs-BymXE4erYP3_N7LsGGWf439j2Htj1Vf80Dr94nn-Rf4c9k9xRuAMpd0RbYGvH0zgSbU4vqkso=rw-e365-nu-w601'
  },
  {
    id: uuidv4(),
    name: 'Pixel 8 a',
    details: 'couleur blue, 64GB, etc ..',
    prix: '499',
    last_prix: '549',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/4mBe7W_xXYt4JNMQHR0wIX7n4Vv6VPxw3MRJWLCiUJAPCoHz5r7oHEKu4wr_KszIouzwCnk7XabMRJ135ae1niTXr6AT0_gRpzE=s1400-w1400-e365-rw-v1'
  },
  {
    id: uuidv4(),
    name: 'Fitbit Charge 6',
    details: 'couleur blue, 64GB, etc ..',
    prix: '119,95',
    last_prix: '159,95',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/1irhYbVhHpMUXbVuRtM2erpOC4BMFhZuplK63D_K6wrUR5Zep6EcbHW-VN81QJV3RwvmHiQRS3FuHOXuTDfiCo0oISYPle-CDjXo=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Google Pixel Watch',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: '/article/montre.webp'
  },
  // Ajoutez d'autres produits ici si nécessaire
];

export const telephones= [
  {
    id: uuidv4(),
    name: 'Pixel 8 pro',
    details: 'couleur blue, 64GB, etc ..',
    long_details: "Contenu de la boîte : Pixel 8 Pro, câble de recharge, adaptateur Quick Switch. L'adaptateur secteur 30 W USB-C est vendu séparément. L'utilisation de certaines fonctionnalités nécessite un compte Google, un accès à Internet et/ou un abonnement payant. Certaines fonctionnalités ne sont pas disponibles dans tous les pays. Pour en savoir plus, consultez la page g.co/pixel/specs.",
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/E2J7hG3ZRWpFOO0NBrkg1s6K1ZKYLwEuCK_lAO7LrPVH3C0eTbFCycYgqs4gr8caAr27S-PKLy2yLhIVGCy4byZ5wZ8kzQpnyQk=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Pixel Buds',
    details: 'couleur blue, 64GB, etc ..',
     long_details: "Contenu de la boîte : Pixel 8 Pro, câble de recharge, adaptateur Quick Switch. L'adaptateur secteur 30 W USB-C est vendu séparément. L'utilisation de certaines fonctionnalités nécessite un compte Google, un accès à Internet et/ou un abonnement payant. Certaines fonctionnalités ne sont pas disponibles dans tous les pays. Pour en savoir plus, consultez la page g.co/pixel/specs.",
    prix: '149',
    last_prix: '229',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/9G5VkratxCGLyYy5NLfL2-AMs-BymXE4erYP3_N7LsGGWf439j2Htj1Vf80Dr94nn-Rf4c9k9xRuAMpd0RbYGvH0zgSbU4vqkso=rw-e365-nu-w601'
  },
  {
    id: uuidv4(),
    name: 'Fitbit Charge 6',
    details: 'couleur blue, 64GB, etc ..',
     long_details: "Contenu de la boîte : Pixel 8 Pro, câble de recharge, adaptateur Quick Switch. L'adaptateur secteur 30 W USB-C est vendu séparément. L'utilisation de certaines fonctionnalités nécessite un compte Google, un accès à Internet et/ou un abonnement payant. Certaines fonctionnalités ne sont pas disponibles dans tous les pays. Pour en savoir plus, consultez la page g.co/pixel/specs.",
    prix: '119,95',
    last_prix: '159,95',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/1irhYbVhHpMUXbVuRtM2erpOC4BMFhZuplK63D_K6wrUR5Zep6EcbHW-VN81QJV3RwvmHiQRS3FuHOXuTDfiCo0oISYPle-CDjXo=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Google Pixel Watch',
    details: 'couleur blue, 64GB, etc ..',
     long_details: "Contenu de la boîte : Pixel 8 Pro, câble de recharge, adaptateur Quick Switch. L'adaptateur secteur 30 W USB-C est vendu séparément. L'utilisation de certaines fonctionnalités nécessite un compte Google, un accès à Internet et/ou un abonnement payant. Certaines fonctionnalités ne sont pas disponibles dans tous les pays. Pour en savoir plus, consultez la page g.co/pixel/specs.",
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: '/article/montre.webp'
  },
  // Ajoutez d'autres produits ici si nécessaire
];
export const pc= [
  {
    id: uuidv4(),
    name: 'PC 8 pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/E2J7hG3ZRWpFOO0NBrkg1s6K1ZKYLwEuCK_lAO7LrPVH3C0eTbFCycYgqs4gr8caAr27S-PKLy2yLhIVGCy4byZ5wZ8kzQpnyQk=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Pixel Buds pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '149',
    last_prix: '229',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/9G5VkratxCGLyYy5NLfL2-AMs-BymXE4erYP3_N7LsGGWf439j2Htj1Vf80Dr94nn-Rf4c9k9xRuAMpd0RbYGvH0zgSbU4vqkso=rw-e365-nu-w601'
  },
  {
    id: uuidv4(),
    name: 'Pixel 8 a',
    details: 'couleur blue, 64GB, etc ..',
    prix: '499',
    last_prix: '549',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/4mBe7W_xXYt4JNMQHR0wIX7n4Vv6VPxw3MRJWLCiUJAPCoHz5r7oHEKu4wr_KszIouzwCnk7XabMRJ135ae1niTXr6AT0_gRpzE=s1400-w1400-e365-rw-v1'
  },
  {
    id: uuidv4(),
    name: 'Fitbit Charge 6',
    details: 'couleur blue, 64GB, etc ..',
    prix: '119,95',
    last_prix: '159,95',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/1irhYbVhHpMUXbVuRtM2erpOC4BMFhZuplK63D_K6wrUR5Zep6EcbHW-VN81QJV3RwvmHiQRS3FuHOXuTDfiCo0oISYPle-CDjXo=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Google Pixel Watch',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: '/article/montre.webp'
  },
  // Ajoutez d'autres produits ici si nécessaire
];
export const tablettes= [
  {
    id: uuidv4(),
    name: 'Pixel 8 pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/E2J7hG3ZRWpFOO0NBrkg1s6K1ZKYLwEuCK_lAO7LrPVH3C0eTbFCycYgqs4gr8caAr27S-PKLy2yLhIVGCy4byZ5wZ8kzQpnyQk=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Pixel Buds pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '149',
    last_prix: '229',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/9G5VkratxCGLyYy5NLfL2-AMs-BymXE4erYP3_N7LsGGWf439j2Htj1Vf80Dr94nn-Rf4c9k9xRuAMpd0RbYGvH0zgSbU4vqkso=rw-e365-nu-w601'
  },
  {
    id: uuidv4(),
    name: 'Pixel 8 a',
    details: 'couleur blue, 64GB, etc ..',
    prix: '499',
    last_prix: '549',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/4mBe7W_xXYt4JNMQHR0wIX7n4Vv6VPxw3MRJWLCiUJAPCoHz5r7oHEKu4wr_KszIouzwCnk7XabMRJ135ae1niTXr6AT0_gRpzE=s1400-w1400-e365-rw-v1'
  },
  {
    id: uuidv4(),
    name: 'Fitbit Charge 6',
    details: 'couleur blue, 64GB, etc ..',
    prix: '119,95',
    last_prix: '159,95',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/1irhYbVhHpMUXbVuRtM2erpOC4BMFhZuplK63D_K6wrUR5Zep6EcbHW-VN81QJV3RwvmHiQRS3FuHOXuTDfiCo0oISYPle-CDjXo=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Google Pixel Watch',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: '/article/montre.webp'
  },
  // Ajoutez d'autres produits ici si nécessaire
];
export const accessoires= [
  {
    id: uuidv4(),
    name: 'Pixel 8 pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/E2J7hG3ZRWpFOO0NBrkg1s6K1ZKYLwEuCK_lAO7LrPVH3C0eTbFCycYgqs4gr8caAr27S-PKLy2yLhIVGCy4byZ5wZ8kzQpnyQk=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Pixel Buds pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '149',
    last_prix: '229',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/9G5VkratxCGLyYy5NLfL2-AMs-BymXE4erYP3_N7LsGGWf439j2Htj1Vf80Dr94nn-Rf4c9k9xRuAMpd0RbYGvH0zgSbU4vqkso=rw-e365-nu-w601'
  },
  {
    id: uuidv4(),
    name: 'Pixel 8 a',
    details: 'couleur blue, 64GB, etc ..',
    prix: '499',
    last_prix: '549',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/4mBe7W_xXYt4JNMQHR0wIX7n4Vv6VPxw3MRJWLCiUJAPCoHz5r7oHEKu4wr_KszIouzwCnk7XabMRJ135ae1niTXr6AT0_gRpzE=s1400-w1400-e365-rw-v1'
  },
  {
    id: uuidv4(),
    quantity: 1,
    name: 'Fitbit Charge 6',
    details: 'couleur blue, 64GB, etc ..',
    prix: '119,95',
    last_prix: '159,95',
    url: 'https://lh3.googleusercontent.com/1irhYbVhHpMUXbVuRtM2erpOC4BMFhZuplK63D_K6wrUR5Zep6EcbHW-VN81QJV3RwvmHiQRS3FuHOXuTDfiCo0oISYPle-CDjXo=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Google Pixel Watch',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: '/article/montre.webp'
  },
  // Ajoutez d'autres produits ici si nécessaire
];
export const montres= [
  {
    id: uuidv4(),
    name: 'Pixel 8 pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/E2J7hG3ZRWpFOO0NBrkg1s6K1ZKYLwEuCK_lAO7LrPVH3C0eTbFCycYgqs4gr8caAr27S-PKLy2yLhIVGCy4byZ5wZ8kzQpnyQk=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Pixel Buds pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '149',
    last_prix: '229',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/9G5VkratxCGLyYy5NLfL2-AMs-BymXE4erYP3_N7LsGGWf439j2Htj1Vf80Dr94nn-Rf4c9k9xRuAMpd0RbYGvH0zgSbU4vqkso=rw-e365-nu-w601'
  },
  {
    id: uuidv4(),
    name: 'Pixel 8 a',
    details: 'couleur blue, 64GB, etc ..',
    prix: '499',
    last_prix: '549',
    url: 'https://lh3.googleusercontent.com/4mBe7W_xXYt4JNMQHR0wIX7n4Vv6VPxw3MRJWLCiUJAPCoHz5r7oHEKu4wr_KszIouzwCnk7XabMRJ135ae1niTXr6AT0_gRpzE=s1400-w1400-e365-rw-v1'
  },
  {
    id: uuidv4(),
    name: 'Fitbit Charge 6',
    details: 'couleur blue, 64GB, etc ..',
    prix: '119,95',
    last_prix: '159,95',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/1irhYbVhHpMUXbVuRtM2erpOC4BMFhZuplK63D_K6wrUR5Zep6EcbHW-VN81QJV3RwvmHiQRS3FuHOXuTDfiCo0oISYPle-CDjXo=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Google Pixel Watch',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: '/article/montre.webp'
  },
  // Ajoutez d'autres produits ici si nécessaire
];
export const ecouteurs= [
  {
    id: uuidv4(),
    name: 'Pixel 8 pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/E2J7hG3ZRWpFOO0NBrkg1s6K1ZKYLwEuCK_lAO7LrPVH3C0eTbFCycYgqs4gr8caAr27S-PKLy2yLhIVGCy4byZ5wZ8kzQpnyQk=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Pixel Buds pro',
    details: 'couleur blue, 64GB, etc ..',
    prix: '149',
    last_prix: '229',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/9G5VkratxCGLyYy5NLfL2-AMs-BymXE4erYP3_N7LsGGWf439j2Htj1Vf80Dr94nn-Rf4c9k9xRuAMpd0RbYGvH0zgSbU4vqkso=rw-e365-nu-w601'
  },
  {
    id: uuidv4(),
    name: 'Pixel 8 a',
    details: 'couleur blue, 64GB, etc ..',
    prix: '499',
    last_prix: '549',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/4mBe7W_xXYt4JNMQHR0wIX7n4Vv6VPxw3MRJWLCiUJAPCoHz5r7oHEKu4wr_KszIouzwCnk7XabMRJ135ae1niTXr6AT0_gRpzE=s1400-w1400-e365-rw-v1'
  },
  {
    id: uuidv4(),
    name: 'Fitbit Charge 6',
    details: 'couleur blue, 64GB, etc ..',
    prix: '119,95',
    last_prix: '159,95',
    quantity: 1,
    url: 'https://lh3.googleusercontent.com/1irhYbVhHpMUXbVuRtM2erpOC4BMFhZuplK63D_K6wrUR5Zep6EcbHW-VN81QJV3RwvmHiQRS3FuHOXuTDfiCo0oISYPle-CDjXo=rw-e365-w1200'
  },
  {
    id: uuidv4(),
    name: 'Google Pixel Watch',
    details: 'couleur blue, 64GB, etc ..',
    prix: '799',
    last_prix: '1023',
    quantity: 1,
    url: '/article/montre.webp'
  },
  // Ajoutez d'autres produits ici si nécessaire
];
